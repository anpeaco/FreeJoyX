...
Full history on the [Releases](https://github.com/anpeaco/FreeJoyX/releases) page. The current wire-format generation is `FIRMWARE_VERSION 0x0070` (dedicated shift buttons). Upgrading a board from an older generation **factory-resets** it on first boot; the configurator reads the old config first and migrates it forward, so re-writing it after the flash restores your mapping.
